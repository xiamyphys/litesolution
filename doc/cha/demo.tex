...
\begin{choice}{D}[加速度]
    ...
    \end{tasks}
\end{choice}
\solute{质点的加速度$a=\frac{\d^2 x}{\d t^2}=-12t\ (\mathrm{SI})<0$.\sokka{D}}
...
\begin{tikzpicture}
    ...
    \pic["$\theta$", draw=red!80!green!60!, ultra thick, angle eccentricity=1.35, angle radius=24]{angle=a--o--b};
...
\vfill
\end{paracol}
\solute{下落高度$h$时木块的速度大小$v=\sqrt{2gh}$.此时重力功率$P=\boldsymbol{F}\cdot\boldsymbol{v}=mg\sin{\theta}\sqrt{2gh}$.\sokka{D}}
...
\begin{paracol}{2}
    ...
    \vfill
\end{paracol}
\solute{由于重力、弹簧弹力均为保守力，所以其做功与路径无关，只与始末位置有关.\sokka{B}}
...
\begin{paracol}{2}
    ...
    \vfill
\end{paracol}
\solute{由于绳子绷紧后拉力始终指向$O$点，故小球对$O$点角动量守恒，即$mv_0h=mvl$. 由于动能和速度平方成正比，所以动能之比为$\frac{E_k}{E_{k_0}}=\frac{v^2}{v_0^2}=\frac{h^2}{l^2}$.}

\begin{paracol}{2}
\begin{problem}[转动定律，圆周运动][4]
...
\end{paracol}
\begin{solution}
    本题第一空考查高斯定理的概念；第二空对带电线$x\sim x+\d x$的一段线元，其电荷量$\d q=\lambda\d x$，与$P$点的距离为$R-x$，在$P$处产生的场强大小为$\d E=\frac{1}{4\pi\varepsilon_0}\frac{\d q}{(R-x)^2}$，对其积分得
    $$
    \begin{aligned}
        E&=\int_{-d/2}^{d/2} \frac{1}{4\pi\varepsilon_0}\frac{\lambda\d x}{(R-x)^2}=\frac{\lambda d}{4\pi\varepsilon_0\ab(R^2-d^2/4)}
    \end{aligned}
    $$
